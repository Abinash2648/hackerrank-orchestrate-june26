...
This solution uses a hybrid rule-based claim verification pipeline.
...
to determine whether minimum evidence requirements are met.

### Image Verification

A Gemini-based image verification module was implemented in image_analyzer.py.
...
Due to Gemini free-tier quota exhaustion during development, the final run used a rule-based fallback mode.

The Gemini module can be enabled by uncommenting the analyze_images() call in main.py.
...
- image verification (when enabled)
